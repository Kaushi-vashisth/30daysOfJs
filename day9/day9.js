...
document.getElementById('name').innerHTML='Hi i am Shakshi';

//Task 2 Select an HTML Element by its class and change its backgroundcolor
let coln=document.getElementsByClassName('age');
for(let i=0;i<coln.length;i++)
{
    coln[i].style.backgroundColor='blue';
}
...
divele.textContent='What would you like to have?'
let ulist = document.getElementById('ulist');
document.body.insertBefore(divele, ulist);

//Task 4 Create a new li element and add it to an existing ul list
let liel=document.createElement('li');
liel.innerHTML='Icecream';
document.getElementById('ulist').appendChild(liel);
document.getElementById('ulist').innerHTML+='<li>Cold coffee</li>';
document.getElementById('ulist').innerHTML+='<li>Tea</li>';
